...
def _build_model_args(config, n_vars, device):
    ...
    return SimpleNamespace(
        input_len    = 512,          # dummy — encoder handles any T via unfold
...
def _encode(model, x):
    ...
    x = model.encoder(x, is_mask=False) # [B*C, P, d_model]
    x = x.reshape(B, C, P, model.d_model)
...
def anomaly_detection(config, checkpoint_path, anomaly_train, anomaly_test,
    ...
    args  = _build_model_args(config, n_vars, device)
    ...
    if linear_probe:
        ...
        print(f"  [TimeDart anomaly] head_lr={head_lr}  encoder_lr={enc_lr}")
    ...
    print(format_table(m, title="TimeDART Anomaly Detection"))
