...
def anomaly_detection(self, path, anomaly_train, anomaly_test,
    ...
    if linear_probe:
        ...
        print(f"  [LE-JEPA anomaly] MODE: linear probe — encoder FROZEN")
    else:
        for p in self.encoder.parameters():
            p.requires_grad = True
        print(f"  [LE-JEPA anomaly] MODE: full fine-tune — encoder UNFROZEN")
    ...
    if linear_probe:
        ...
        print(f"  [LE-JEPA anomaly] head_lr={head_lr}  encoder_lr={enc_lr}")
    ...
    print(format_table(m, title="LE-JEPA Anomaly Detection"))
