...
from models.patch_transformer import PatchTransformerWrapper
...
class TS2Vec:
    ...
    def __init__(
        ...
        soft_instance = False,
        soft_temporal = False,
        patch_len=None,        # set to use PatchTransformerWrapper instead of TSEncoder
        patch_n_layers=3,
        patch_n_heads=8,
        patch_d_ff=256,
    ):
        ...
        while True:
            ...
            for x, idx in train_loader:
                ...
                if self.patch_len is not None:
                    # Work in patch units so slicing aligns with encoder output
                    pl = self.patch_len
                    P = ts_l // pl
                    crop_l = np.random.randint(low=2 ** (self.temporal_unit + 1), high=P + 1)
                    crop_left = np.random.randint(P - crop_l + 1)
                    crop_right = crop_left + crop_l
                    crop_eleft = np.random.randint(crop_left + 1)
                    crop_eright = np.random.randint(low=crop_right, high=P + 1)
                    crop_offset = np.random.randint(low=-crop_eleft, high=P - crop_eright + 1, size=x.size(0))
                    out1_all = self._net(take_per_row(x, (crop_offset + crop_eleft) * pl, (crop_right - crop_eleft) * pl))
                    out2_all = self._net(take_per_row(x, (crop_offset + crop_left) * pl, (crop_eright - crop_left) * pl))
                else:
