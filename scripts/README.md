...
anything — each reads the frozen backbones `run_per_model.sh` produced, so run
that first for the task a figure needs.
...
| Stage | Figure | Reads |
| --- | --- | --- |
| `isotropy` | effective rank vs the forecasting gain over random | forecast backbones |
| `drift` | latent drift under time-shift and time-masking | forecast backbones |
| `tsne` | t-SNE of classification embeddings | classify backbones |
| `gaussianity` | LE-JEPA embedding Gaussianity | an LE-JEPA classify backbone |

The four Python entry points ship with different defaults for seed, GPU and
pre-training source — t-SNE defaults to the hybrid corpus, drift to seed 1337 —
so the wrapper passes all of them explicitly. Both `mae`/`patchtst` and
`diffusion`/`timedart` are accepted on `--models`.
