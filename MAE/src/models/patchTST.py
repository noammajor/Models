...
def _proj(in_dim, out_dim, dropout, mlp_head=False, hidden_dim=512):
    ...
    For MLP, dropout is INSIDE the block. Callers replace external dropout with
    nn.Identity() when mlp_head=True so total dropout count = 1.
    """
    if not mlp_head:
...
class PatchTSTEncoder(nn.Module):
    ...
    def _get_pos_enc(self, seq_len: int) -> Tensor:
        ...
        Slices if shorter than W_pos, interpolates if longer.
        """
